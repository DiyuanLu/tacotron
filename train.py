...
from load_audio import get_batch
...
class Graph:
    # Load vocabulary 
    #char2idx, idx2char = prep.load_vocab()
    ...
    def __init__(self, is_training=True):
        ...
        with self.graph.as_default():
            ...
            if is_training:
                ...
                self.spectro, self.magnit, self.length, self.num_batch = get_batch()    # Get data batch
            else: # Evaluation
                self.length = tf.placeholder(tf.int32, shape=(None, None))
                self.spectro = tf.placeholder(tf.float32, shape=(None, None, hp.n_mels*hp.r))

            self.decoder_inputs = shift_by_one(self.spectro)   # this is the decoder's input
            ...
            with tf.variable_scope("net"):
                ...
                #### Decoder
                self.outputs1 = net.decode1(self.decoder_inputs, 
                                         self.memory, # encoder RNN output
                                         is_training=is_training) # (N, T', hp.n_mels*hp.r)
                self.outputs2 = net.decode2(self.outputs1, is_training=is_training) # (N, T', (1+hp.n_fft//2)*hp.r)
            ...
            if is_training:  
                ...
                if hp.loss_type=="l1": # L1 loss
                    ...
                    self.loss1 = tf.squared_difference(self.outputs1, self.spectro)
                ...
                # Logging  
                ## histograms
                self.expected1_h = tf.reduce_mean(tf.reduce_mean(self.spectro, -1), 0)
                self.got1_h = tf.reduce_mean(tf.reduce_mean(self.outputs1, -1),0)
                
                self.expected2_h = tf.reduce_mean(tf.reduce_mean(self.magnit, -1), 0)
                self.got2_h = tf.reduce_mean(tf.reduce_mean(self.outputs2, -1),0)
                
                ## images
                self.expected1_i = tf.expand_dims(tf.reduce_mean(self.spectro[:1], -1, keep_dims=True), 1)
                self.got1_i = tf.expand_dims(tf.reduce_mean(self.outputs1[:1], -1, keep_dims=True), 1)
                
                self.expected2_i = tf.expand_dims(tf.reduce_mean(self.magnit[:1], -1, keep_dims=True), 1)
